...
export default function DBBrowser({ preselect = {} }) {
  ...
  useEffect(() => {
    fetch('/api/diag/dbs').then(r => r.json()).then(setDbs).catch(e => setErr(e.message))
  }, [])
  ...
  useEffect(() => {
    ...
    setFacts(null)
    setFactsErr(null)
    fetch(`/api/diag/dbs/${encodeURIComponent(dbId)}/containers/${encodeURIComponent(container)}/facts`)
  ...
  async function loadHistory(factId) {
    const r = await fetch(`/api/diag/dbs/${encodeURIComponent(dbId)}/containers/${encodeURIComponent(container)}/facts/${factId}/history`)
